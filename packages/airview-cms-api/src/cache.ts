...
async function get(key: string): Promise<any | undefined> {
...
function set(key: string, value: any, expireSecs?: number): Promise<void> {
  return new Promise((resolve, reject) => {
    const ts =
      expireSecs == undefined ? null : new Date().getTime() + expireSecs * 1000;
    _db.run(
...
async function query(branch: string, query: string) {
  return new Promise((resolve, reject) => {
    _db.all(
